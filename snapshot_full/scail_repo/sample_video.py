...
def _get_model_time_settings(args):
    num_frames = None
    time_compress = None
    try:
        network_params = args.model_config.network_config.params
        num_frames = int(network_params.num_frames)
        time_compress = int(network_params.time_compressed_rate)
    except Exception:
        pass
    if time_compress is None and getattr(args, "vae_compress_size", None):
        try:
            time_compress = int(args.vae_compress_size[0])
        except Exception:
            pass
    return num_frames, time_compress


def _get_max_latent_frames(args):
    num_frames, time_compress = _get_model_time_settings(args)
    if num_frames and time_compress:
        return (num_frames - 1) // time_compress + 1, time_compress
    return None, time_compress
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            if args.image2video:    # i2v 输入是图片+prompt
                ...
                max_latent_frames, time_compress = _get_max_latent_frames(args)
                if not time_compress:
                    time_compress = 1
                if max_latent_frames:
                    max_input_frames = max_latent_frames * time_compress
                    if pose_video.shape[0] > max_input_frames:
                        pose_video = pose_video[:max_input_frames]
                ...
                max_latent_frames, _ = _get_max_latent_frames(args)
                if max_latent_frames and pose_latent.shape[1] > max_latent_frames:
                    pose_latent = pose_latent[:, :max_latent_frames]
                    if "smpl" in args.representation:
                        smpl_render_latent = smpl_render_latent[:, :max_latent_frames]
                    if image.shape[1] > max_latent_frames:
                        image = image[:, :max_latent_frames]
